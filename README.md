...
The project allows to create an SVG NFT with on-chain metadata that reflects the owner mood.
...
```
git clone https://github.com/edvardg/foundry-nft-on-chain-metadata.git
cd foundry-nft-on-chain-metadata
forge install
forge build
```
...
After deploy to a testnet or local net, you can run the scripts to mint an NFT and flip the NFT mood.

Using cast:

```
cast send <NFT_CONTRACT_ADDRESS> "mintNft()" --private-key <PRIVATE_KEY> --rpc-url $SEPOLIA_RPC_URL
```
```
cast send <NFT_CONTRACT_ADDRESS> "flipMood(uint256)" <TOKEN_ID> --private-key <PRIVATE_KEY> --rpc-url $SEPOLIA_RPC_URL
```
...
```
```
make flipNft ARGS="--network sepolia"
```
